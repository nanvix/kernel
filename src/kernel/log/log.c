 ...
 *
 */
static size_t chkbuf(char *buffer, size_t buf_pos, size_t ins_size)
{
 ...
 *
 */
static size_t cpy2buf(char *buffer, const char *s, size_t i)
{
 ...
 * @return The next position to be inserted in the buffer.
 */
static size_t fmt_prefix(char *buffer, const char *file, const char *funcname,
                         int level)
{
    size_t i = 0;

    // Inserts message level.
    i = cpy2buf(buffer, MSG_LEVEL[level], i);

    // Inserts module name.
    i = cpy2buf(buffer, LOG_MODULE_SCOPE, i);
    buffer[i++] = '[';
    ...
    i = chkbuf(buffer, i, __strlen(funcname) + 6);
    buffer[i++] = ' ';
    i = cpy2buf(buffer, funcname, i);
    i = cpy2buf(buffer, "(): ", i);
...
int __log(const char *file, const char *funcname, unsigned level, ...)
{
    ...
    size_t i = fmt_prefix(buffer, file, funcname, level);

    va_start(args, level);

    // Print message into the buffer.
    const char *fmt_str = va_arg(args, char *);
    i = chkbuf(buffer, i, __strlen(fmt_str));
